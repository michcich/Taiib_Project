...
namespace Gym.BLL_EF.Repositories
{
    public class ClubTicketTypeRepository : Repository<ClubTicketType>, IClubTicketTypeRepository
    {
        ...
        {
            return await _context.ClubTicketTypes.Where(ctt => ctt.ClubId == clubId).ToListAsync();
        }

        public async Task<List<ClubTicketType>> GetAllAsync()
        {
            return await _context.ClubTicketTypes.ToListAsync();
        }
    }
}
